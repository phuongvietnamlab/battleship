...
- **Bố trí hạm đội kéo-thả**: 5 tàu (sân bay 5, thiết giáp 4, tuần dương 3, ngầm 3, khu trục 2) trên lưới 11×11; xoay ngang/dọc; hỗ trợ cảm ứng.
- Bắn theo lượt, đếm ô trúng, phát hiện tàu chìm, màn thắng/thua, **chơi lại giữ tỉ số**. Người đi trước đổi so le mỗi ván.
- **Đồng hồ lượt 45 giây** (server-authoritative): hết giờ → bỏ lượt; bỏ lượt 3 lần liên tiếp (~2 phút lặng) → **xử thua**. Vòng tròn đếm ngược trên scoreboard, đỏ + nhấp nháy khi ≤10s.
- **Scoreboard** hiển thị **avatar + tên + điểm** của cả 2 người (đồng bộ hồ sơ FB), viền sáng ở người đang tới lượt.
- **Chat trong phòng**: tin nhắn ephemeral + 8 emoji nhanh, badge tin chưa đọc (ẩn khi đấu máy).
...
- **Frontend**: React 18 (1 file `public/app.jsx`), bundle bằng **esbuild** thành `app.js` self-contained (không CDN ngoài, trừ SDK `fbinstant`). Lớp i18n EN/VI ngay trong bundle.
...
- `TURN_MS = 45000` (đồng hồ lượt) · `MAX_TIMEOUTS = 3` (bỏ lượt liên tiếp → xử thua)
...
- **Giới hạn dev FB**: dưới build dev chưa provision, `getASIDAsync`/`getID` có thể trả null/UNKNOWN → auto-resume theo FB id không khả dụng; game vẫn chạy nhờ fallback localStorage + nhập mã phòng tay.
